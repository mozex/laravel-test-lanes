...
- Known harmless quirk from the spec: Illuminate's `TestViews` process hooks run in the parallel runner's own mini-application, which re-registers the token resolver with the RAW worker index, so compiled-view directories are created and cleaned under `test_{index}` while tests read `test_lane{n}`. Blade recompiles, nothing breaks, but it is a dead cleanup path and quiet disk growth. No fix shipped; revisit if view-cache disk use ever matters.
...
- The wubly project (`C:\Work\www\wubly`) is the origin of this design; its `tests/TestLane.php` spec comment carries the full research record (prior art, measured connection budget, traps).
